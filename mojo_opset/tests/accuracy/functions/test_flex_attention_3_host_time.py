...
def _print_perf_stats(label, times):
    """打印 host 时间统计 (ms): iters / avg / min / max / median。"""
    if not times:
        print(f"[perf] {label}: no samples")
        return
    n = len(times)
    avg = sum(times) / n
    s = sorted(times)
    med = s[n // 2] if n % 2 == 1 else (s[n // 2 - 1] + s[n // 2]) / 2
    print(f"[perf] {label}: iters={n} avg={avg*1e3:.3f}ms min={min(times)*1e3:.3f}ms "
          f"max={max(times)*1e3:.3f}ms median={med*1e3:.3f}ms")

def _print_perf_stats_no_min_no_max(label, times):
    """打印 host 时间统计 (ms): 去除最小/最大值后的 avg/min/max/median, 并打印所有原始值。"""
    if not times:
        print(f"[perf] {label}: no samples")
        return
    n = len(times)
    s = sorted(times)
    # 去除 1 个最小值和 1 个最大值, 避免异常抖动拉偏均值
    if n >= 3:
        trimmed = s[1:-1]
    else:
        trimmed = s
    m = len(trimmed)
    avg = sum(trimmed) / m
    med = trimmed[m // 2] if m % 2 == 1 else (trimmed[m // 2 - 1] + trimmed[m // 2]) / 2
    raw_ms = [f"{t*1e3:.3f}" for t in times]
    print(f"[perf] {label}: samples={n} used={m} avg={avg*1e3:.3f}ms "
          f"min={min(trimmed)*1e3:.3f}ms max={max(trimmed)*1e3:.3f}ms "
          f"median={med*1e3:.3f}ms")
    print(f"[perf] {label} all (ms): [{', '.join(raw_ms)}]")
...
def test_flex_attention(batch_size,q_head, kv_head, head_dim, data_lens, data_types, sliding_windows, global_windows, dtype, mask_func):
    ...
    _sync()
    # ---- 前向 host 时间统计 (warmup + 多次平均) ----
    # 每轮用全新 clone 输入, 避免计算图累积; 最后一轮的 q_mojo/mojo_output
    # 供后续精度比对与反向计时使用。
    _FWD_WARMUP = 3
    _FWD_ITERS = 12
    fwd_times = []
    for _i in range(_FWD_WARMUP + _FWD_ITERS):
        q_mojo = q_base.detach().clone().requires_grad_(True)
        k_mojo = k_base.detach().clone().requires_grad_(True)
        v_mojo = v_base.detach().clone().requires_grad_(True)
        _sync()
        ...
        if _i >= _FWD_WARMUP:
            fwd_times.append(_t1 - _t0)
        if _i < _FWD_WARMUP + _FWD_ITERS - 1:
            del mojo_output, q_mojo, k_mojo, v_mojo
            gc.collect()
            torch.npu.empty_cache()
    _print_perf_stats_no_min_no_max("flex_attention fwd", fwd_times)

    # ---- 反向 host 时间统计 (warmup + 多次平均) ----
    # 反向会消费计算图, 每轮需先 (untimed) 重跑前向重建图, 再计时反向。
    # 最后一轮的梯度供后续精度比对使用。
    _BWD_WARMUP = 3
    _BWD_ITERS = 12
    bwd_times = []
    for _i in range(_BWD_WARMUP + _BWD_ITERS):
        # 重建计算图 (不计入反向耗时)
        q_mojo = q_base.detach().clone().requires_grad_(True)
        k_mojo = k_base.detach().clone().requires_grad_(True)
        v_mojo = v_base.detach().clone().requires_grad_(True)
        ...
        _t1 = time.perf_counter()
        if _i >= _BWD_WARMUP:
            bwd_times.append(_t1 - _t0)
        if _i < _BWD_WARMUP + _BWD_ITERS - 1:
            del mojo_output, q_mojo, k_mojo, v_mojo
            gc.collect()
            torch.npu.empty_cache()
    _print_perf_stats_no_min_no_max("flex_attention bwd", bwd_times)
    ...
    _sync()
    mojo_output.float().mean().backward(return_grid,retain_graph=True)
    #mojo_output.float().mean().backward(return_grid)
    _sync()
